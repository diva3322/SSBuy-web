...
try {
    ...
    // [修正重點] 在迴圈外先讀取一次舊資料，用於判斷是否為新遊戲
    // 雖然我們是智慧生成，但判斷 "新增" vs "修改" 還是需要舊資料
    // 這裡我們以舊的 games.json 為基準
    let existingData = {};
    if (fs.existsSync(gamesDataPath)) {
        try {
            existingData = JSON.parse(fs.readFileSync(gamesDataPath, 'utf8'));
        } catch(e) {
            console.log("警告：舊的 games.json 格式錯誤，將視為全部新增。");
        }
    }
    // [修正重點結束]
    ...
    for (const gameName in gamesData) {
        ...
        if (giftCodeInfo) {
            ...
            if (!fs.existsSync(giftcodeOutputPath)) {
                ...
                if (!newGames.includes(gameName)) newGames.push(gameName); // 確保兩邊都記錄
                changesMade++;
            } else if (fs.readFileSync(giftcodeOutputPath, 'utf8') !== giftcodePageHtml) {
                fs.writeFileSync(giftcodeOutputPath, giftcodePageHtml);
                if (!updatedGames.includes(gameName)) updatedGames.push(gameName); // 確保兩邊都記錄
                changesMade++;
            }
        }
    }
